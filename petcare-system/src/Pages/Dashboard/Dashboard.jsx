...
// NOW receives todayAttendance, attendanceSaved, onAttendanceSaved from App
function Dashboard({
  pets, vets, appointments, records, petStatuses,
  todayAttendance, attendanceSaved, onAttendanceSaved
}) {
  const [showAttendance, setShowAttendance] = useState(false);

  async function handleSaveAttendance(entries) {
    await saveAttendance(entries);
    // build map and bubble up to App
    const log = {};
    entries.forEach(e => { log[e.vetId] = e.isPresent; });
    onAttendanceSaved(log);
  }
        ...
        <div className={styles.statsGrid}>
          ...
          <Card className={styles.statCard} style={{ '--accent': '#3bb8d4' }}>
            <div className={styles.statTop}>
              <div className={styles.statIconCircle} style={{ background: 'rgba(59,184,212,0.12)', color: '#3bb8d4' }}>
          ...
          <Card className={styles.statCard} style={{ '--accent': '#38b98a' }}>
            <div className={styles.statTop}>
              <div className={styles.statIconCircle} style={{ background: 'rgba(56,185,138,0.12)', color: '#38b98a' }}>
            ...
            <div className={styles.statBar}>
              <div className={styles.statBarFill} style={{ width: totalPets ? `${(inExam / totalPets) * 100}%` : '0%', background: '#38b98a' }} />
            </div>
          </Card>

          <Card className={styles.statCard} style={{ '--accent': '#8b7cf6' }}>
            <div className={styles.statTop}>
              <div className={styles.statIconCircle} style={{ background: 'rgba(139,124,246,0.12)', color: '#8b7cf6' }}>
            ...
            <div className={styles.statBar}>
              <div className={styles.statBarFill} style={{ width: todayAppts.length ? '75%' : '0%', background: '#8b7cf6' }} />
            </div>
          </Card>
              ...
              return (
                <div key={v.id} className={styles.staffRow}>
                  <div
                    ...
                    style={{ background: present ? '#0d2b36' : '#e8f0f4', color: present ? '#fff' : '#7a9baa' }}
                  >
                    {getInitials(v.name)}
                  </div>
                  ...
                  <div key={pet.id} className={styles.patientRow}>
                    <div className={styles.patientInfo}>
                      <div className={styles.patientName}>
                        {getPetEmoji(pet?.petType?.typeName)} {pet.name}
                      </div>
                      <div className={styles.patientMeta}>
                        {pet.owner?.name || '—'}{vet ? ` · ${vet.name}` : ''}
                      </div>
                    </div>
                ...
                <div key={appt.id} className={styles.apptRow}>
                  ...
                  <div className={styles.apptInfo}>
                    <div className={styles.apptName}>
                      {getPetEmoji(pet?.petType?.typeName)} {pet?.name || 'Unknown Pet'}
                    </div>
              ...
              return (
                <div key={rec.id} className={styles.recRow}>
                  <div className={styles.recInfo}>
                    ...
                    <div className={styles.recMeta}>{vet?.name || '—'} · {new Date(rec.examinationDate).toLocaleDateString('en-US', {
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric'
                    })}</div>
